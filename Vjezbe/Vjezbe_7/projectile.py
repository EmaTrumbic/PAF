...
class Projectile:
    def __init__(self, x, y, v, kut, m, p, Cd, A, dt):
        ...
        self.lista_ax = [-1*np.sign(self.lista_vx[-1])*(self.p*self.Cd*self.A/(2*self.m))*(self.lista_vx[-1])**2]
        self.lista_ay = [-9.81-np.sign(self.lista_vy[-1])*(self.p*self.Cd*self.A/(2*self.m))*(self.lista_vy[-1])**2]
    ...
    def __moveEuler(self):
        while self.lista_y[-1]>=0:
            ...
            self.lista_ax.append(-1*np.sign(self.lista_vx[-1])*(self.p*self.Cd*self.A/(2*self.m))*(self.lista_vx[-1])**2)
            self.lista_ay.append(-9.81-np.sign(self.lista_vy[-1])*(self.p*self.Cd*self.A/(2*self.m))*(self.lista_vy[-1])**2)

    def __moveRungeKutta(self):
        while self.lista_y[-1]>=0:
            k_vx = [(-1*np.sign(self.lista_vx[-1])*(self.p*self.Cd*self.A/(2*self.m))*(self.lista_vx[-1])**2)*self.dt]
            k_vy = [(-9.81-np.sign(self.lista_vy[-1])*(self.p*self.Cd*self.A/(2*self.m))*(self.lista_vy[-1])**2)*self.dt]
            k_x = [self.lista_vx[-1]*self.dt]
            k_y = [self.lista_vy[-1]*self.dt]
            for i in range(3):
                k_vx.append((-1*np.sign(self.lista_vx[-1]+0.5*k_vx[-1])*(self.p*self.Cd*self.A/(2*self.m))*(self.lista_vx[-1]+0.5*k_vx[-1])**2)*self.dt)
                k_vy.append((-9.81-np.sign(self.lista_vy[-1]+0.5*k_vy[-1])*(self.p*self.Cd*self.A/(2*self.m))*(self.lista_vy[-1]+0.5*k_vy[-1])**2)*self.dt)
